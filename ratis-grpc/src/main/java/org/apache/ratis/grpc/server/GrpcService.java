...
public final class GrpcService extends RaftServerRpcWithProxy<GrpcServerProtocolClient,
  ...
  private GrpcService(RaftServer raftServer, Supplier<RaftPeerId> idSupplier,
    ...
    super(idSupplier, id -> new PeerProxyMap<>(id.toString(),
        //主动发起请求的GrpcServerProtocolClient，发给server
    ...
    //这里就是把GrpcServerProtocolService注册到server中，让server监听
    //GrpcServerProtocolService继承的RaftServerProtocolServiceImplBase，也就是RaftServerProtocolService
    ...
    //如果adminPort和serverPort一致，那就一起注册到一个Server中
    if (!separateAdminServer) {
      //注册GrpcAdminProtocolService服务，继承自AdminProtocolServiceImplBase
      addAdminService(raftServer, serverBuilder);
    }
    //如果clientPort和serverPort一致，那就一起注册到一个Server中
    if (!separateClientServer) {
      //注册GrpcClientProtocolService服务，继承自RaftClientProtocolServiceImplBase
