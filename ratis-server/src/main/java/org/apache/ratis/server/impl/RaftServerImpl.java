...
class RaftServerImpl implements RaftServer.Division,
  ...
  /**
   * 这个类是一个方法类，主要用来获取RaftServerImpl的状态（当前角色、运行状态、任期等）
   */
  ...
  /**
   * 代理类，这个类可以代理多个不同group组的不同raftServer实例，维护<groupId,raftServerImpl>映射
   */
  private final RaftServerProxy proxy;
  /**
   * 对应业务场景的状态，比如计数器场景，主要维护count变量，那就设计一个和count相关的状态机
   */
  private final StateMachine stateMachine;
  /**
   * 获取raftServerImpl当前状态
   */
  private final Info info =  new Info();

  /**
   * 保存着raftServer属性集合 division？
   */
  ...
  /**
   * 这是作为raftClient主动发起请求的类
   * 带有记忆功能的
   */
  private final MemoizedSupplier<RaftClient> raftClient;

  /**
   * 客户端请求缓存类，当客户端失效时，可以用这个缓存类
   */
  private final RetryCacheImpl retryCache;
  /**
   * CommitInfo缓存，concurrentHashMap，主要缓存commitIndex
   */
  private final CommitInfoCache commitInfoCache = new CommitInfoCache();

  /**
   * raft实例的管理适配器
   * 用于JConsole查看该raft的id、leaderId、currentTerm、groupId、role、follower、groupsOfSingleNode信息
   * 便于调试
   */
  private final RaftServerJmxAdapter jmxAdapter;
  /**
   * 更新和选举相关的参数，比如选举次数、选举超时次数
   */
  ...
  /**
   * 正在进行安装快照（发送快照给滞后节点）请求的索引号，如果不为空，说明正在进行快照请求
   * 不能进行其他操作（比如appendEntries）
   */
  ...
  //保证先把startAsFollower执行了
  /**
   * 当raft启动后会将该变量置为true，当为false，不会提供任何服务，如appendEntries
   */
  ...
  boolean start() {
    ...
    //Configuration 主要是节点信息（地址、id啥的）(从state中拿到的)
    ...
    if (conf != null && conf.containsInBothConfs(getId())) {
      ...
      //启动FollowState线程，检测心跳是否超时
      //状态转换Starting->Running
      startAsFollower();
    } else {//如果没有配置的话，就默默置为follower，没有资格检测心跳
      LOG.info("{}: start with initializing state, conf={}", getMemberId(), conf);
      startInitializing();
    }

    //注册该raft的id、leaderId、currentTerm、groupId、role、follower、groupsOfSingleNode信息
    //便于JConsole调试
    registerMBean(getId(), getMemberId().getGroupId(), jmxAdapter, jmxAdapter);
    //启动状态机更新者线程，用来更新状态机
    state.start();
    //保证startAsFollower执行了
  ...
  static boolean registerMBean(
    ...
    //在jmx中的格式是：Ratis:service=RaftServer,group=XXX,id=XXX
  ...
  private void startAsFollower() {
    ...
    //启动followerState，让这个线程干follower该干的事
  ...
  synchronized void changeToCandidate(boolean forceStartLeaderElection) {
    ...
    // start election
    //这里raft状态必须是New才可以
    //forceStartLeaderElection：是否跳过预投票
  ...
  /**
   *
   * @param phase 是预请求、还是真投票
   * @param candidateId
   * @param candidateGroupId
   * @param candidateTerm
   * @param candidateLastEntry
   * @return
   * @throws IOException
   */
  private RequestVoteReplyProto requestVote(Phase phase,
    ...
    synchronized (this) {
      ...
      } else if(shouldSendShutdown(candidateId, candidateLastEntry)) {//如果配置不包括candidate，那就shut down
        shouldShutdown = true;
      }
      //组装reply，最重要的是term
  ...
  /**
   * 一致性检查，确保leader的previous log和本节点的last log一致
   * @param expectedTerm
   * @param previous
   * @param entries
   */
  ...
  private void preAppendEntriesAsync(RaftPeerId leaderId, RaftGroupId leaderGroupId, long leaderTerm,
    ...
    //确保本节点是staring或running状态
    assertLifeCycleState(LifeCycle.States.STARTING_OR_RUNNING);

    //确保服务器启动起来了，而且还是follower
    ...
    try {
      //检测previousLog是否真是logEntries的上一条日志条目
  ...
  private long checkInconsistentAppendEntries(TermIndex previous, LogEntryProto... entries) {
    ...
    // 检查是否正在通过状态机安装快照。
    //installSnapshot的意思是发送快照给滞后的follower
    //termIndex是快照的最后一个已提交日志索引
  ...
  /**
   * 这个RPC请求用于实现leader优先级没有本节点高，且最后的日志条目索引号也没有本节点新（高）
   * 那么leader就会让这个follower去选举，自己退下
   * @param request
   * @return
   * @throws IOException
   */
  ...
  public StartLeaderElectionReplyProto startLeaderElection(StartLeaderElectionRequestProto request) throws IOException {
    ...
    synchronized (this) {
      ...
      //只有对方确实是leader、且自己是follower、且leaderLastEntry<this.LastEntry就可以转为candidate
  ...
  /**
   * RaftServer的监控适配器，实现RaftServerMXBean接口，从RaftServerImpl拿数据
   * 然后将自己注册进去
   */
